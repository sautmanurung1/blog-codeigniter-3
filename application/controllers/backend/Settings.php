...
class Settings extends CI_Controller{
	...
	function update(){
	    ...
	    if(!empty($_FILES['logo_header']['name']) && !empty($_FILES['logo_footer']['name'])){
	        ...
	        $this->site_model->update_information($site_id,$site_name,$site_title,$site_description,$logo_header,$logo_footer,$facebook,$twitter,$linkedin,$instagram,$pinterest);
	        $this->session->set_flashdata('msg','success');
	        redirect('backend/settings');

	    }elseif(!empty($_FILES['logo_header']['name']) && empty($_FILES['logo_footer']['name'])){
	    	if ($this->upload->do_upload('logo_header')){
	            $img_header = $this->upload->data();
	            $logo_header=$img_header['file_name'];
	        }
